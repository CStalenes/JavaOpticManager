...
create view tabTech as(
    select p.nom , p.prenom, t.diplome, m.adresse, m.telephone, count(p.idpersonne) as nbTech
    from Personne p , Technicien t, Magasin m
    where t.idPersonne = p.idPersonne
    group by p.nom, p.prenom, t.diplome
