...
public class PanelLunettes extends PanelDeBase implements ActionListener{
		...
		this.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(e.getClickCount() == 2)//vsi le nombre de clic sur le resultat = 2 alors on supprimera
				{
					...
					int retour = JOptionPane.showConfirmDialog(null,"Voulez-vous supprimer la lunette",
				...
				else if(e.getClickCount() == 1)//vsi le nombre de clic sur le resultat = 1 alors on modifie
				{
					...
					int idverre = Integer.parseInt(unTableau.getValueAt(numLigne, 8).toString());// j 'ai fait ça pour convertir idverre en int comme idlunette
	...
	public void traitement (int choix) {
		...
		int idverre = Integer.parseInt(tab[0]);// pour la clef etrangere de ta table c'est pareille faut faire ça aussi
